...
function groupByDay(
  ...
  const map = new Map<
    string,
    { p50: number[]; p95: number[]; p99: number[]; total: number }
  >();
  for (const r of rows) {
...
export default function PerformanceTab({
  ...
  const avgP50 = daily.length
    ? daily.reduce((a, d) => a + d.p50, 0) / daily.length
    : 0;
  const avgP95 = daily.length
    ? daily.reduce((a, d) => a + d.p95, 0) / daily.length
    : 0;
  const avgP99 = daily.length
    ? daily.reduce((a, d) => a + d.p99, 0) / daily.length
    : 0;
  ...
  const pipelineItems = stageRow
    ? PIPELINE_STAGE_KEYS.map((key) => {
        const avgKey =
          `avg_${key.replace("stage_", "")}` as keyof typeof stageRow;
    ...
    <div>
      <div className="astat-grid">
        ...
        <StatCard
          ...
          note={`${rps.toFixed(3)} RPS`}
        />
        <StatCard
          label="Total Request"
          icon={Activity}
          color="indigo"
          value={fmtThousand(totalReq)}
          note={`${days} hari terakhir`}
        />
      </div>
        ...
        <div className="apanel apanel-full">
          ...
          <LineChart
            ...
            series={[
              {
                name: "P50",
                values: daily.map((d) => d.p50),
                color: "var(--chart-purple)",
              },
              {
                name: "P95",
                values: daily.map((d) => d.p95),
                color: "var(--chart-amber)",
              },
              {
                name: "P99",
                values: daily.map((d) => d.p99),
      ...
      <div className="apanel-row">
        <div className="apanel apanel-full">
          ...
          <p className="apanel-desc">
            Klik tahap untuk melihat request paling lambat di tahap tersebut,
            lengkap dengan session &amp; pertanyaan.
          </p>
          {stageBreakdown.loading ? (
            <p style={{ fontSize: 12.5, color: "var(--gray-400)" }}>Memuat…</p>
          ) : stageBreakdown.error ? (
            <p
              role="alert"
              style={{ fontSize: 12.5, color: "var(--coral-500)" }}
            >
              {stageBreakdown.error}
            </p>
            ...
            <p style={{ fontSize: 12.5, color: "var(--gray-400)" }}>
              Belum ada data latency pada periode ini.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="apanel-row">
        <div className="apanel apanel-full">
          ...
          <p className="apanel-desc">
            Klik baris untuk melihat detail lengkap request.
          </p>
                ...
                {recentRows.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        <span
                          className={`status-badge ${STATUS_BADGE_CLASS[dstatus]}`}
                        >
