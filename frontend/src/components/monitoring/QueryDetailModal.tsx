...
import { EvaluationDialog } from '@/components/evaluation/EvaluationUI';
...
export default function QueryDetailModal({
  ...
  useEffect(() => {
    ...
    getRequestDetail(requestId)
      .then((response) => {
        if (!cancelled)
          setResult({ id: requestId, data: response.data, error: null });
      })
      .catch((error) => {
        if (!cancelled)
          setResult({
  ...
  return (
    <EvaluationDialog title="Detail request" onClose={onClose} wide>
      <p className="atable-mono">{requestId}</p>
        ...
        {data && dstatus && (
          <>
            ...
            <div className="detail-question-block">
              <p className="detail-question-text">
                &ldquo;{data.question || '(tidak ada teks pertanyaan)'}&rdquo;
              </p>
              <span className={`status-badge ${STATUS_BADGE_CLASS[dstatus]}`}>
            ...
            <div className="detail-info-grid">
              ...
              <div className="info-field">
                ...
                <span className="info-value atable-mono">
                  {data.session_id || '—'}
                </span>
              </div>
              <div className="info-field">
              ...
              <div className="info-field">
                ...
                <span className="info-value">
                  {formatDomain(data.domain_detected)}
                </span>
              </div>
              <div className="info-field">
                <span className="info-label">
            ...
            <div className="detail-section">
              <h4>
                <ClipboardCheck style={{ width: 14, height: 14 }} /> Penilaian
                untuk Evaluasi RAG
              </h4>
              <CaseReview key={requestId} requestId={requestId} />
            </div>
            ...
            {data.status === 'error' && (
              <div className="detail-error-box">
                <div className="deb-title">
                  {formatErrorSource(data.error_source)}
                </div>
                <div className="deb-detail">
                  {deriveErrorDetail(data.error_source, data.error_type)}
                </div>
                {data.openai_retry_count > 0 && (
                  <div className="deb-retry">
                    Retry OpenAI: {data.openai_retry_count}x sebelum gagal
                  </div>
                )}
              </div>
            )}
            ...
            <div className="detail-section">
              <h4>
                <Layers style={{ width: 14, height: 14 }} /> Breakdown Latency
                per Tahap
              </h4>
              <HBarList
                items={PIPELINE_STAGE_KEYS.map((key) => ({
                  ...
                  value: data[key] ?? 0,
                  valueLabel:
                    data[key] != null ? fmtMs(data[key]) : 'tidak dilalui',
                }))}
                numbered
              />
            </div>
            ...
            {data.retrieval_detail && data.retrieval_detail.length > 0 && (
              <div className="detail-section">
                <h4>
                  <FileSearch style={{ width: 14, height: 14 }} /> Detail
                  Cross-Encoder ({data.retrieval_detail.length} kandidat)
                </h4>
                      ...
                      {[...data.retrieval_detail]
                        ...
                        .map((c) => (
                          <tr key={c.parent_id}>
                            ...
                            <td className="atable-mono">
                              {c.score.toFixed(4)}
                            </td>
                            <td>
                              <span
                                className={`status-badge ${c.accepted ? 'status-success' : 'status-neutral'}`}
                              >
            ...
            {data.is_no_relevant_doc && (
              <div className="detail-section">
                <div className="empty-state" style={{ padding: '18px 14px' }}>
                  <p style={{ margin: 0 }}>
                    Tidak ada dokumen relevan yang ditemukan untuk pertanyaan
                    ini.
                  </p>
                </div>
              </div>
