import type { Domain, ErrorSource, QueryLogRow, RequestStatus, StageKey } from './monitoringTypes';
import { PIPELINE_STAGE_KEYS } from './monitoringTypes';

// ============================================================
// Ambang batas & mapping — bagian ini KEPUTUSAN DESAIN eksplisit
// karena backend tidak menyimpan konsep "lambat"/"timeout" secara
// langsung (hanya 'success'/'error'/'quota_rejected'). Ubah di sini
// kalau ambang batasnya perlu disesuaikan.
// ============================================================

/** Di atas ini (ms), request sukses dianggap "lambat" secara visual. */
export const SLOW_LATENCY_THRESHOLD_MS = 3000;

export type DisplayStatus = 'success' | 'lambat' | 'timeout' | 'error' | 'quota_rejected';
...
export function deriveDisplayStatus(row: {
  ...
  if (row.total_ms != null && row.total_ms > SLOW_LATENCY_THRESHOLD_MS) return 'lambat';
  return 'success';
}

export const STATUS_BADGE_CLASS: Record<DisplayStatus, string> = {
  success: 'status-success',
  lambat: 'status-warning',
...
export function deriveErrorDetail(source: ErrorSource, errorType: string | null): string {
  if (!source) return errorType || 'Terjadi kesalahan yang tidak terklasifikasi.';
  const type = errorType || '';
  if (type.toLowerCase().includes('timeout')) return `${type} — permintaan melebihi batas waktu.`;
  if (source === 'rate_limit') return 'Permintaan ditolak OpenAI karena rate limit tercapai.';
...
export function fmtCompact(n: number | null | undefined): string {
  ...
  if (Math.abs(v) >= 1000) return (v / 1000).toFixed(v % 1000 === 0 ? 0 : 1).replace('.', ',') + 'rb';
  return String(v);
}
