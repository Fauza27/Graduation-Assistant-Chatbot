...
🔐 REQUEST VALIDATION & RATE LIMIT (SlowAPI + Security Headers)
    ↓ [📊 MONITORING: new_collector(), start_stage("validation")]
🛡️ ENHANCED INPUT VALIDATION & SANITIZATION
    ↓
📊 QUOTA SERVICE CHECK (Shared Service - Daily Limits)
    ↓ [📊 MONITORING: end_stage("validation"), persist_quota_rejection() jika ditolak]
💾 LOAD/CREATE SESSION MEMORY (Strategy Pattern)
    ↓ [📊 MONITORING: start/end stage("session_load")]
🔤 QUERY NORMALIZATION & REFORMULATION
    ↓ [📊 MONITORING: start/end stage("reformulation"), set_field(rewrite_method)]
🔍 HYBRID RETRIEVAL (Vector + BM25)
    ↓ [📊 MONITORING: start/end stage("embedding"), start/end stage("retrieval"),
       set_field(num_docs_retrieved, embedding_tokens, embedding_cost_usd)]
🔗 PARENT ASSEMBLY
    ↓ [📊 MONITORING: start/end stage("parent_assembly"), set_field(domain_detected)]
🎯 CROSS-ENCODER RERANKING
    ↓ [📊 MONITORING: start/end stage("reranking"), set_field(num_docs_after_rerank,
       top/avg_cross_encoder_score, retrieved_parent_ids, retrieval_detail)]
🤖 LLM GENERATION (GPT-4o-mini)
    ↓ [📊 MONITORING: start/end stage("generation"), set_field(input_tokens,
       output_tokens, llm_cost_usd) dari actual usage API]
💾 SAVE MEMORY & CHAT LOG (Strategy-based Storage)
    ↓ [📊 MONITORING: start/end stage("db_save")]
📤 RESPONSE TO USER
    ↓ [📊 MONITORING: persist_metrics() → INSERT ke request_metrics]
```

**Arsitektur Utama**: **Retrieval-First** (Evidence-Driven) dengan **Layered Security**, **Optimized Service Architecture**, dan **Built-in Monitoring & Observability** — Sistem langsung melakukan pencarian dokumen tanpa klasifikasi intent terlebih dahulu, menggunakan adaptive history management berdasarkan ketersediaan konteks, diperkuat dengan security middleware terintegrasi, shared service architecture, dan sistem monitoring non-invasif yang mencatat metrics per request ke database.
...
│   │   ├── admin_metrics.py         # Admin monitoring metrics APIs (NEW)
...
│   │   ├── context.py               # Context-based metrics collector (INTI)
│   │   ├── errors.py                # Error taxonomy & classification
│   │   ├── openai_client.py         # Instrumented HTTP client untuk retry counter
│   │   ├── pricing.py               # Kalkulasi cost dari token usage
│   │   └── writer.py                # Persist metrics ke Supabase
...
│   ├── pricing.yaml                 # OpenAI API pricing (NEW)
│   └── section_keywords.yaml       # Document classification
├── 📁 scripts/                      # Database migrations
│   ├── supabase.sql                 # Main schema setup
│   ├── supabase_session_migration.sql
│   ├── supabase_migration_observability.sql        # Tabel request_metrics (NEW)
│   ├── supabase_migration_observability_views.sql  # SQL views monitoring (NEW)
...
**9. `request_metrics`** (Monitoring & Observability per Request — NEW)

Tabel ini mencatat data monitoring lengkap untuk setiap request chat, digunakan untuk analisis performa, kualitas retrieval, biaya API, dan troubleshooting.

- `id` (bigint, PK, AUTO-INCREMENT): ID unik.
- `request_id` (uuid, NOT NULL): UUID unik per request.
- `created_at` (timestamptz, NOT NULL, DEFAULT now()): Waktu request.
- `session_id` (text): ID sesi percakapan.
- `mahasiswa_id` (text): ID mahasiswa jika via website, null jika Telegram.
- `channel` (text): `'website'` | `'telegram'`.
- `status` (text): `'success'` | `'error'` | `'quota_rejected'`.
- `error_type` (text): Nama class exception Python (mis. `"OpenAIError"`).
- `error_source` (text): `'openai'` | `'supabase'` | `'validation'` | `'rate_limit'` | `'unknown'`.
- `stage_validation_ms` (numeric): Durasi tahap validasi auth dalam ms.
- `stage_session_load_ms` (numeric): Durasi load session memory dalam ms.
- `stage_reformulation_ms` (numeric): Durasi query reformulation dalam ms (null jika tidak triggered).
- `stage_embedding_ms` (numeric): Durasi embedding query dalam ms.
- `stage_retrieval_ms` (numeric): Durasi hybrid search RPC dalam ms.
- `stage_reranking_ms` (numeric): Durasi cross-encoder reranking dalam ms.
- `stage_parent_assembly_ms` (numeric): Durasi fetch parent documents dalam ms.
- `stage_generation_ms` (numeric): Durasi LLM generation dalam ms.
- `stage_db_save_ms` (numeric): Durasi save session & chat log dalam ms.
- `total_ms` (numeric): Total durasi request end-to-end dalam ms.
- `num_docs_retrieved` (int): Jumlah chunk hasil hybrid search sebelum rerank.
- `num_docs_after_rerank` (int): Jumlah dokumen akhir setelah threshold reranking.
- `top_cross_encoder_score` (numeric): Skor tertinggi cross-encoder.
- `avg_cross_encoder_score` (numeric): Rata-rata skor cross-encoder.
- `domain_detected` (text): `'PI'` | `'KKP'` | `'SKRIPSI'` | `'NON_SKRIPSI'` | `'UNKNOWN'`.
- `is_no_relevant_doc` (boolean): True jika tidak ada dokumen relevan ditemukan.
- `retrieved_parent_ids` (text[]): Array parent_id yang masuk context LLM.
- `rewrite_method` (text): `'None'` | `'Rule'` | `'LLM'`.
- `input_tokens` (int): Actual input tokens dari OpenAI API response.
- `output_tokens` (int): Actual output tokens dari OpenAI API response.
- `embedding_tokens` (int): Estimasi token query untuk embedding.
- `llm_cost_usd` (numeric): Biaya generation LLM dalam USD.
- `embedding_cost_usd` (numeric): Biaya embedding dalam USD.
- `openai_retry_count` (int): Jumlah retry ke OpenAI (semua 4 titik panggilan).
- `question` (text): Teks pertanyaan asli user — diisi sedini mungkin.
- `username` (text): Nama tampilan user (dari JWT/profil Telegram).
- `retrieval_detail` (jsonb): Detail scoring semua kandidat dokumen.
...
```json
{
  ...
  "channel": "website",
  "status": "success",
  "stage_embedding_ms": 245.3,
  "stage_retrieval_ms": 312.7,
  "stage_reranking_ms": 489.1,
  "stage_generation_ms": 1823.5,
  "total_ms": 3124.8,
  "domain_detected": "KKP",
  "num_docs_retrieved": 12,
  "num_docs_after_rerank": 3,
  "top_cross_encoder_score": 0.91,
  "is_no_relevant_doc": false,
  "input_tokens": 1450,
  "output_tokens": 220,
  "llm_cost_usd": 0.000349,
  "embedding_cost_usd": 0.0000019,
  "question": "Apa syarat pembimbing KKP?",
...
**Admin Metrics APIs (dengan prefix /api) — NEW:**
- `GET /api/admin/metrics/latency` - Latency percentile (p50/p95/p99) & throughput per jam
- `GET /api/admin/metrics/stage-breakdown` - Rata-rata durasi tiap tahap pipeline per hari
- `GET /api/admin/metrics/errors` - Error rate & quota rejection rate per hari
- `GET /api/admin/metrics/errors/breakdown` - Breakdown error by source (openai/supabase/dll)
- `GET /api/admin/metrics/openai-retry` - Retry rate ke OpenAI per hari
- `GET /api/admin/metrics/retrieval-quality` - Kualitas retrieval per hari (no-doc rate, avg score)
- `GET /api/admin/metrics/top-documents` - Dokumen paling sering diambil sebagai context
- `GET /api/admin/metrics/domain-stats` - Breakdown query per domain (KKP/PI/SKRIPSI/NON_SKRIPSI)
- `GET /api/admin/metrics/cost` - Cost harian LLM + embedding
- `GET /api/admin/metrics/cost/per-user` - Cost per mahasiswa
- `GET /api/admin/metrics/usage/active-users` - Active users harian/bulanan per channel
- `GET /api/admin/metrics/usage/new-vs-returning` - Sesi baru vs lanjutan per hari
- `GET /api/admin/metrics/usage/turns-per-session` - Rata-rata turn per sesi
...
## SECTION MONITORING & OBSERVABILITY (NEW — Increment 4)

Sistem monitoring menggunakan pola **context-based collector** yang non-invasif. Setiap request chat dicatat ke tabel `request_metrics` di Supabase tanpa mengganggu alur chat utama (fail-safe).

### Prinsip Desain

1. **Tanpa infrastruktur baru** — Menggunakan Supabase PostgreSQL yang sudah ada sebagai data store untuk metrics.
2. **Non-invasive** — Tidak mengubah signature fungsi yang sudah ada. Menggunakan `contextvars.ContextVar` supaya instrumentasi bisa "titip data" dari dalam fungsi manapun.
3. **Fail-open** — Kegagalan mencatat metrics TIDAK PERNAH membuat chat gagal. Semua write ke tabel metrics dibungkus try/except.
4. **Single entry point** — Baik request dari website maupun Telegram sama-sama memanggil `ai_services.chat()`. Instrumentasi utama dipasang di sini agar otomatis berlaku untuk kedua channel.

### Modul Monitoring (`src/monitoring/`)

```
src/monitoring/
├── __init__.py          # Module init
├── context.py           # INTI — Context-based metrics collector (contextvars)
├── errors.py            # Error taxonomy: ChatError, classify_exception()
├── openai_client.py     # httpx client berisi event hook retry counter
├── pricing.py           # Kalkulasi LLM & embedding cost dari pricing.yaml
└── writer.py            # persist_metrics() & persist_quota_rejection()
```

### Alur Instrumentasi

```
[ai.py / chat_handler.py]
  └─ new_collector(session_id, channel, question)  ← buat collector di awal request
  └─ start_stage("validation") / end_stage()

[ai_services.py]
  └─ get_current() → ambil collector yang sudah ada
  └─ start_stage("session_load") / end_stage()
  └─ start_stage("reformulation") / end_stage()
  └─ set_field(rewrite_method=...)

[hybrid_search.py]
  └─ start_stage("embedding") / end_stage()
  └─ set_field(embedding_tokens=..., embedding_cost_usd=...)
  └─ start_stage("retrieval") / end_stage()
  └─ set_field(num_docs_retrieved=...)

[pipeline.py]
  └─ set_field(domain_detected=...)
  └─ start_stage("parent_assembly") / end_stage()
  └─ start_stage("reranking") / end_stage()
  └─ _record_final_retrieval_metrics(...)
     → set_field(num_docs_after_rerank, top/avg_score, retrieved_parent_ids, retrieval_detail)

[chain.py]
  └─ set_field(input_tokens, output_tokens, llm_cost_usd)  ← dari actual API response

[ai_services.py — akhir request]
  └─ start_stage("db_save") / end_stage()
  └─ collector.status = "success"
  └─ persist_metrics(collector)  → INSERT ke request_metrics

[ai_services.py — exception handler]
  └─ error_source, error_type = classify_exception(e)
  └─ collector.status = "error"
  └─ persist_metrics(collector)  → tetap catat meski error
```

### Retry Counter (openai_client.py)

Dipasang di **4 titik panggilan OpenAI** via `http_client=build_instrumented_http_client()`:
1. `RAGChain.__init__` + `build_rag_chain()` — generation
2. `IntentClassifier.__init__` — klasifikasi intent
3. `QueryReformulator.__init__` — reformulasi query
4. `HybridSearcher.__init__` — embedding query

Setiap response HTTP 429/5xx menambah `openai_retry_count` di collector aktif.

### SQL Views Agregasi (`scripts/supabase_migration_observability_views.sql`)

| View | Requirement | Keterangan |
|------|-------------|------------|
| `v_latency_stats_hourly` | A1, A3 | p50/p95/p99 latency per jam |
| `v_stage_breakdown_daily` | A2 | Avg durasi tiap stage per hari |
| `v_error_stats_daily` | B1, B4 | Error rate & quota rejection |
| `v_error_breakdown_daily` | B2 | Error by source |
| `v_openai_retry_stats_daily` | B3 | Retry rate ke OpenAI |
...
| `v_avg_turns_per_session_daily` | E2 | Avg turn per sesi |
| `v_followup_rate_daily` | E5 | Follow-up question rate |
| `v_admin_activity_daily` | F3 | Aktivitas admin |

### Konfigurasi

```env
# Di .env
ENABLE_REQUEST_METRICS=true   # Aktifkan/matikan monitoring (default: true)
```

```yaml
# Di config/pricing.yaml
llm:
  gpt-4o-mini:
    input_per_1m: 0.15
    output_per_1m: 0.60
...
-- scripts/supabase_migration_observability.sql      ← tabel + index + RLS
-- scripts/supabase_migration_observability_views.sql ← SQL views agregasi
```
