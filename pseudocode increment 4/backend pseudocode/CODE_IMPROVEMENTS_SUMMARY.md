...
## 🔭 Increment 4 — Monitoring & Observability System

### 5. ✅ Context-Based Request Metrics Collector
**Files**: `src/monitoring/context.py` (new)
- **Implemented** `RequestMetricsCollector` dataclass dengan semua field metrics
- **Menggunakan** `contextvars.ContextVar` untuk thread-safe & async-safe collector
- **Helper functions** module-level: `start_stage()`, `end_stage()`, `set_field()`, `add_retry()`
- **Fail-safe**: semua fungsi no-op jika tidak ada collector aktif

### 6. ✅ Error Taxonomy & Classification
**Files**: `src/monitoring/errors.py` (new), `src/services/ai_services.py` (updated)
- **Dipindahkan** `ChatError` & `RetrievalError` dari `ai_services.py` ke `errors.py`
- **Ditambahkan** `ValidationServiceError`, `OpenAIServiceError`, `SupabaseServiceError`, `RateLimitServiceError`
- **Implemented** `classify_exception()` untuk auto-classify exception pihak ketiga berdasarkan nama modul

### 7. ✅ OpenAI Retry Counter via HTTP Event Hook
**Files**: `src/monitoring/openai_client.py` (new)
- **Implemented** `build_instrumented_http_client()` dengan httpx event hook
- **Dipasang** di semua 4 titik panggilan OpenAI: RAGChain, IntentClassifier, QueryReformulator, HybridSearcher
- **Menghitung** retry dari response HTTP 429/5xx tanpa monkey-patching SDK

### 8. ✅ Actual Token Usage & Cost Tracking
**Files**: `src/monitoring/pricing.py` (new), `config/pricing.yaml` (new), `src/generation/chain.py` (updated)
- **Menggantikan** estimasi tiktoken dengan `response.usage_metadata` dari OpenAI API
- **Kalkulasi cost** berdasarkan `config/pricing.yaml` (mudah diupdate tanpa ubah kode)
- **Tracking** embedding token & cost via tiktoken estimasi di `hybrid_search.py`

### 9. ✅ Metrics Persistence ke Database
**Files**: `src/monitoring/writer.py` (new), `scripts/supabase_migration_observability.sql` (new)
- **Persist** `RequestMetricsCollector` ke tabel `request_metrics` di Supabase
- **`persist_quota_rejection()`** untuk mencatat penolakan kuota sebelum `ai_services.chat()` dipanggil
- **Fail-safe**: kegagalan insert TIDAK PERNAH menggagalkan response ke user

### 10. ✅ End-to-End Pipeline Instrumentation
**Files**: `src/services/ai_services.py`, `src/api/ai.py`, `src/bot/handlers/chat_handler.py`, `src/retrieval/hybrid_search.py`, `src/retrieval/pipeline.py`, `src/generation/chain.py` (semua updated)
- **9 stage timing** diukur: validation, session_load, reformulation, embedding, retrieval, reranking, parent_assembly, generation, db_save
- **Question capture sedini mungkin** di entrypoint untuk memastikan tetap tercatat meski error
- **Domain detection** dari self-query classifier (bukan dari dokumen yang berhasil diambil)
- **Retrieval detail** (JSONB) menyimpan semua kandidat + skor untuk drill-down analysis

### 11. ✅ SQL Views & Admin Metrics API
**Files**: `scripts/supabase_migration_observability_views.sql` (new), `src/api/admin_metrics.py` (new), `application.py` (updated)
- **15+ SQL views** agregasi untuk semua metrics requirements (A1-F3)
- **16 REST endpoints** di `/api/admin/metrics/*` dengan auth admin
- **System overview endpoint** untuk dashboard summary
- **Registrasi router** `admin_metrics` di `application.py`

### Summary Increment 4

| Category | Files New | Files Updated |
|----------|-----------|---------------|
| Monitoring core | 5 | 0 |
| Config | 1 | 1 |
| Database | 2 | 0 |
...
**Key metrics tersedia setelah Increment 4:**
- Latency p50/p95/p99 per channel per jam
- Error rate & breakdown by source
- OpenAI retry rate
- Retrieval quality (no-doc rate, avg cross-encoder score)
- LLM & embedding cost per request/user/day
- Active users & session analytics
